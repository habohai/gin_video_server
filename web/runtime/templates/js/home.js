	...
	$("#uploadform").on('submit', function(e) {
		e.preventDefault()
	  	...
	  	createVideo(vname, function(res, err) {
			...
			$.ajax({
				url : 'http://' + window.location.hostname + ':80/api/v1/upload/' + obj['id'],
				//url:'http://127.0.0.1:80/upload/dbibi',
				type : 'POST',
				data : formData,
...
function initPage(callback) {
	getUserId(function(res, err) {
		...
		listAllVideos(function(res, err) {
			...
			var obj = JSON.parse(res);
			listedVideos = obj['videoinfos'];
			obj['videoinfos'].forEach(function(item, index) {
				var ele = htmlVideoListElement(item['id'], item['video_name'], item['created_on']);
				$("#items").append(ele);
			});
			callback();
		});
...
function selectVideo(vid) {
	var url = 'http://' + window.location.hostname + ':80/api/v1/videos/'+ vid
		var video = $("#curr-video");
		$("#curr-video:first-child").attr('src', url);
		$("#curr-video-name").text(currentVideo['name']);
		$("#curr-video-ctime").text('Uploaded at: ' + mydate(currentVideo['created_on']));
		//currentVideoId = vid;
		refreshComments(vid);
}
...
function refreshComments(vid) {
	listAllComments(vid, function (res, err) {
		...
		obj['comments'].forEach(function(item, index) {
			var ele = htmlCommentListElement(item['id'], item['user_id'], item['content']);
...
function registerUser(callback) {
	...
	var pwd = $("#pwd").val();
	var apiUrl = window.location.hostname + ':80/api/v1/api';
	...
	var dat = {
		'url': 'http://'+ window.location.hostname + ':8000/api/v1/user',
		'method': 'POST',
	...
	$.ajax({
		url  : 'http://' + window.location.hostname + ':80/api/v1/api',
		type : 'post',
...
function signinUser(callback) {
	...
	var dat = {
		'url': 'http://'+ window.location.hostname + ':8000/api/v1/user/' + username,
		'method': 'POST',
		'req_body': JSON.stringify(reqBody)
	};
...
function getUserId(callback) {
	var dat = {
		'url': 'http://' + window.location.hostname + ':8000/api/v1/user/' + uname,
		'method': 'GET'
	};
...
function createVideo(vname, callback) {
	...
	var dat = {
		'url': 'http://' + window.location.hostname + ':8000/api/v1/user/' + uname + '/videos',
		'method': 'POST',
		'req_body': JSON.stringify(reqBody)
	};
...
function deleteVideo(vid, callback) {
  var dat = {
    'url': 'http://' + window.location.hostname + ':8000/api/v1/user/' + uname + '/videos/' + vid,
    'method': 'DELETE',
    'req_body': ''
  };
...
function postComment(vid, content, callback) {
 var reqBody = {
  ...
  var dat = {
    'url': 'http://' + window.location.hostname + ':8000/api/v1/videos/' + vid + '/comments',
    'method': 'POST',
    'req_body': JSON.stringify(reqBody)
  };
